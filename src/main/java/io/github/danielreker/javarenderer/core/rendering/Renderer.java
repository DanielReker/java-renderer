...
public class Renderer<VS_IO extends VertexShaderIoBase, FS_IO extends FragmentShaderIoBase> {

    private final ShaderProgram<VS_IO, FS_IO> shaderProgram;

    private final PolygonClipper<VS_IO> polygonClipper;

    private final TriangleRasterizer<VS_IO, FS_IO> triangleRasterizer;


    public Renderer(ShaderProgram<VS_IO, FS_IO> shaderProgram) {
    ...
    public void render(
        ...
        List<VS_IO> processedVertices = vbo
                .stream()
                .map(vertexObject -> {
                    VS_IO vsIo = shaderProgram.createAndPrepareVertexIO(vertexObject);
    ...
    private void assembleAndRasterizeTriangles(
            List<VS_IO> allProcessedVertices,
            FrameBuffer targetFrameBuffer
    ) {
        for (int i = 0; i < allProcessedVertices.size() - 2; i += 3) {
            final List<VS_IO> clippedVertices = polygonClipper
